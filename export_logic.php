...
require_once __DIR__.'/export_logic_core.php';
...
$stmt = $pdo->prepare("
    SELECT 
        fg.name AS className, 
        fg.gibbonSchoolYearID, 
        sy.name AS schoolYearName, 
        sy.firstDay, 
        sy.lastDay
    FROM gibbonFormGroup fg
    JOIN gibbonSchoolYear sy 
      ON fg.gibbonSchoolYearID = sy.gibbonSchoolYearID
...
foreach ($classes as $class) {
    ...
    $invoiceCount = (int)$checkStmt->fetchColumn();
    if ($invoiceCount > 0) {
...
echo "<style>
    ...
    .empty-note { margin: 12px 0 24px; color: #b71c1c; }
</style>";

if (empty($classes)) {
    echo "<div class='empty-note'><strong>Không có lớp nào có hóa đơn phù hợp với bộ lọc hiện tại.</strong></div>";
}
