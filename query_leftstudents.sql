...
JOIN
	(SELECT gibbonStudentEnrolment.*, MAX(gibbonSchoolYearID) as yearID
 		FROM gibbonStudentEnrolment GROUP BY gibbonPersonID) gibbonSE
    ON (student.gibbonPersonID = gibbonSE.gibbonPersonID)
...
WHERE student.gibbonRoleIDPrimary = '003'
    ...
    AND (student.dateStart IS NULL OR student.dateStart <= CURDATE())
    AND (student.dateEnd <= CURDATE())
ORDER BY center,
         student.username,
         student.surname,
