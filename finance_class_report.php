...
function getMonthFromBillingName($name) {
    preg_match('/\d{1,2}/', (string)$name, $matches);
...
if (isset($_GET['export']) && $_GET['export'] == '1') {
    include __DIR__ . '/export_logic.php';
    exit;
}
...
.note { color:#b71c1c; margin-top:10px; }
</style>";
...
if ($formGroup === '') {
    echo "<div class='note'><strong>Please select class.</strong></div>";
    exit;
}

$teacherName = '';
if (!empty($formGroup) && !empty($gibbonSchoolYearID)) {
    $stmt = $pdo->prepare("
        SELECT CONCAT(p.surname, ' ', p.preferredName) AS teacherName
        FROM gibbonFormGroup fg
        JOIN gibbonPerson p ON fg.gibbonPersonIDTutor = p.gibbonPersonID
...
$sql = "
    ...
    WHERE f.name = :formGroup 
      AND i.status LIKE 'Paid%%' 
      AND i.gibbonSchoolYearID = :schoolYearID
";
...
foreach ($rows as $r) {
    ...
    if (!empty($startDate) && !empty($endDate)) {
        ...
        if ($paidTS >= $filterStart && $paidTS <= $filterEnd) {
            $cell = number_format((float)$r['paidAmount'], 0);
            if (!isset($totalPerMonth[$key])) $totalPerMonth[$key] = 0;
            $totalPerMonth[$key] += (float)$r['paidAmount'];
        } else {
...
foreach ($months as $m) {
    echo "<th>".safe($m['month'])."/".safe($m['year'])."</th>";
}
echo "</tr>";
