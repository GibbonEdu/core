...
function getStaffDashboardContents($connection2, $guid, $gibbonPersonID)
{
    ...
    while ($rowRollGroups = $resultRollGroups->fetch()) {
        ...
        $rollGroups[$count][2] .= "<a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/Roll Groups/rollGroupExport.php&gibbonRollGroupID='.$rowRollGroups['gibbonRollGroupID']."'>".__('Export to Excel')."<img style='margin-left: 5px' title='".__('Export to Excel')."' src='./themes/".$_SESSION[$guid]['gibbonThemeName']."/img/download.png'/></a>";
