...
function getParentalDashboardContents($connection2, $guid, $gibbonPersonID) {
	...
	$class="error" ;
	if (!($updateReturn=="")) {
		switch ($updateReturn) { // converted from if..elif to switch..case
		case "fail0":
					...
					if ($resultSub->rowCount()!=1) {
						...
						$gradesOutput.="<td>" ;
							...
							if ($resultWork->rowCount()>0) {
								...
								switch($rowWork["type"]) { // converted from if..elif..else to switch..case..default
								case "File":
					...
					if ($step!="") {
						$output.="<tr>" ;
							$output.="<td style='text-align: justify; font-size: 125%; border-bottom: 2px solid #fff; background-color: rgba(255,255,255,0.25); padding: 15px 4px' colspan=5>" ;
								switch ($step) { // converted from if..elif to switch..case
								case 1:
...
function getParentPhotoUploader($connection2, $guid) {
	...
	if ($category=="Parent") {
		...
		$class="error" ;
		if (!($deleteReturn=="")) {
			switch ($deleteReturn) { // converted from if..elif to switch..case
			case "fail1":
		...
		$class="error" ;
		if (!($uploadReturn=="")) {
			switch ($uploadReturn) { // converted from if..elif to switch..case
			case "fail1":
...
function sidebar($connection2, $guid) {
	...
	$loginReturnMessage="" ;
	if (!($loginReturn=="")) {
		switch ($loginReturn) { // converted from if..elif to switch..case
		case "fail0b":
	...
	if (isset($_SESSION[$guid]["username"])) {
		if (count($_SESSION[$guid]["gibbonRoleIDAll"])>1 AND $_SESSION[$guid]["address"]=="") {
			...
			$class="error" ;
			if (!($switchReturn=="")) {
				switch ($switchReturn) { // converted from if..elif to switch..case
				case "fail0":
...
function printRollGroupTable($guid, $gibbonRollGroupID, $columns, $connection2, $confidential=TRUE, $orderBy="Normal") {
	try {
		...
		switch ($orderBy) {
		if ($orderBy=="surname") { // converted from if..elif..else to switch..case..default
		case "surname":
	...
	if ($highestAction=="View Student Profile_full") {
		...
		if ($gibbonAlertLevelID!="") {
			...
			if ($alert!=FALSE) {
				...
				$output.="<a style='font-size: " . $fontSize . "px; color: #" . $alert["color"] . "; text-decoration: none' href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/Students/student_view_details.php&gibbonPersonID=" . $gibbonPersonID . "&subpage=Behaviour Record'><div title='$title' style='float: right; text-align: center; vertical-align: middle; max-height: " . $height . "px; height: " . $height . "px; width: " . $width . "px; border-top: 2px solid #" . $alert["color"] . "; margin-right: 2px; background-color: #" . $alert["colorBG"] . "'>" . _('B') . "</div></a>" ;
			}
		}
