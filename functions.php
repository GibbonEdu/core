...
function getParentDashboardContents($connection2, $guid, $gibbonPersonID)
{
    ...
    if (isSchoolOpen($guid, $date, $connection2) == true and isActionAccessible($guid, $connection2, '/modules/Planner/planner.php') and $_SESSION[$guid]['username'] != '') {
        ...
        if ($result->rowCount() > 0) {
            $plannerOutput .= "<div style='margin-top: 20px'><span style='font-size: 85%; font-weight: bold'>".__('Today\'s Classes')."</span> . <span style='font-size: 70%'><a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/Planner/planner.php&search='.$gibbonPersonID."'>".__('View Planner').'</a></span></div>';
...
function getParentPhotoUploader($connection2, $guid)
{
    ...
    if ($category == 'Parent') {
        ...
        $output .= '<br/><a class="button" href="'.$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/ID Cards/photos.php" style="border: 1px solid #222222;background-color: #eeeeee;;color: #444444;font-weight: bold;font-size: 13px ;text-decoration:none;padding: 8px;display:block;text-align:center;">';
    ...
    if ($highestAction == 'View Student Profile_full' or $highestAction == 'View Student Profile_fullNoNotes') {
        ...
        try {
            ...
            $sqlSports = "SELECT DISTINCT gibbonActivityStudent.gibbonActivityID FROM gibbonActivity 
                    ...
                    AND gibbonActivity.type='Sports'
        ...
        if ($resultSports->rowCount() > 0) {
            ...
            $output .= "<div title='$title' style='font-size: ".$fontSize.'px; float: right; text-align: center; vertical-align: middle; max-height: '.$height.'px; height: '.$height.'px; width: '.($width*1.2).'px; border-top: 2px solid '.$color.'; margin-left: 2px; color: '.$color.'; background-color: '.$colorBG.";font-variant:small-caps;'>Sp</span></div>";
            $output .= "</a>";
        }
