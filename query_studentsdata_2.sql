SELECT student.username,
       ...
       student.Religion,
       (CASE
            WHEN (@find := LOCATE('s:3:"001";s', student.fields)) > 0 THEN REPLACE(LEFT(@var :=SUBSTRING(student.fields, @find + 15),LOCATE('";', @var)-1), '"', '')
            ELSE ''
        END) AS Caste,
       gibbonFamily.homeAddress AS Address,
       (CASE
            WHEN (@find := LOCATE('s:3:"002";s', student.fields)) > 0 THEN REPLACE(LEFT(@var := SUBSTRING(student.fields, @find + 15), LOCATE('";', @var)-1), '"', '')
            ELSE ''
        END) AS 'Exact DOB',
       student.dob,
       student.dateStart AS 'Date of joining',
       (CASE
            WHEN (@find := LOCATE('s:3:"005";s', student.fields)) > 0 THEN REPLACE(LEFT(@var := SUBSTRING(student.fields, @find + 15), LOCATE('";', @var)-1), '"', '')
            ELSE ''
        END) AS 'Students ocupation',
       (CASE
            WHEN (@find := LOCATE('s:3:"006";s', student.fields)) > 0 THEN REPLACE(LEFT(@var := SUBSTRING(student.fields, @find + 15), LOCATE('";', @var)-1), '"', '')
            ELSE ''
        END) AS 'Salary per month',
       (CASE
            WHEN (@find := LOCATE('s:3:"007";s', student.fields)) > 0 THEN REPLACE(LEFT(@var := SUBSTRING(student.fields, @find + 15), LOCATE('";', @var)-1), '"', '')
            ELSE ''
        END) AS 'Work start time',
       (CASE
            WHEN (@find := LOCATE('s:3:"008";s', student.fields)) > 0 THEN REPLACE(LEFT(@var := SUBSTRING(student.fields, @find + 15), LOCATE('";', @var)-1), '"', '')
            ELSE ''
        END) AS 'Work end time',
       student.emergency1name AS 'Guardian 1 Name',
       student.emergency1Relationship AS 'Guardian 1 Relationship',
       student.emergency1Number1 AS 'Guardian 1 Contact Number',
       student.emergency1Number2 AS 'Guardian 1 Contact Number 2',
       ...
       student.emergency2Number2 AS 'Guardian 2 Contact Number 2',

       CONCAT(adult1.firstName, ' ', adult1.surname) AS 'Adult 1 Name',
       relation1.relationship AS 'Adult 1 Relationship',
       (CASE
            WHEN (@find := LOCATE('s:3:"023";s', adult1.fields)) > 0 THEN REPLACE(LEFT(@var := SUBSTRING(adult1.fields, @find + 15), LOCATE('";', @var)-1), '"', '')
            ELSE ''
        END) AS 'Adult 1 Education',
       adult1.phone1 AS 'Adult 1 Number',
       adult1.profession AS 'Adult 1 Profession',
       (CASE
            WHEN (@find := LOCATE('s:3:"006";s', adult1.fields)) > 0 THEN REPLACE(LEFT(@var := SUBSTRING(adult1.fields, @find + 15), LOCATE('";', @var)-1), '"', '')
            ELSE ''
        END) AS 'Adult 1 Salary',
       CONCAT(adult2.firstName, ' ', adult2.surname) AS 'Adult 2 Name',
       relation2.relationship AS 'Adult 2 Relationship',
       (CASE
            WHEN (@find := LOCATE('s:3:"023";s', adult2.fields)) > 0 THEN REPLACE(LEFT(@var := SUBSTRING(adult2.fields, @find + 15), LOCATE('";', @var)-1), '"', '')
            ELSE ''
        END) AS 'Adult 2 Education',
       adult2.phone1 AS 'Adult 2 Number',
       adult2.profession AS 'Adult 2 Profession',
       (CASE
            WHEN (@find := LOCATE('s:3:"006";s', adult2.fields)) > 0 THEN REPLACE(LEFT(@var := SUBSTRING(adult2.fields, @find + 15), LOCATE('";', @var)-1), '"', '')
            ELSE ''
        END) AS 'Adult 2 Salary',
       (CASE
            WHEN (@find := LOCATE('s:3:"004";s', student.fields)) > 0 THEN REPLACE(LEFT(@var := SUBSTRING(student.fields, @find + 15), LOCATE('";', @var)-1), '"', '')
            ELSE ''
        END) AS 'Family income',
       (CASE
            WHEN (@find := LOCATE('s:3:"003";s', student.fields)) > 0 THEN REPLACE(LEFT(@var := SUBSTRING(student.fields, @find + 15), LOCATE('";', @var)-1), '"', '')
            ELSE ''
        END) AS 'Family size',
       (CASE
            WHEN (@find := LOCATE('s:3:"011";s', student.fields)) > 0 THEN REPLACE(LEFT(@var := SUBSTRING(student.fields, @find + 15), LOCATE('";', @var)-1), '"', '')
            ELSE ''
        END) AS 'Is the student in another shool',
       (CASE
