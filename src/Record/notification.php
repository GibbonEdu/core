	...
	public function set($personID, $text, $moduleName = '', $actionLink)
	{
		...
		if ($this->insert || $archived)
		{
			...
			if ($pObj->rowCount() == 1 && ! $pObj->isEmpty('email') && $pObj->getField('receiveNotificationEmails') == 'Y')
			{
				...
				$body .= trans::__(array('If you do not wish to receive email notifications from %1$s, please %2$sclick here%3$s to adjust your preferences:', array($this->view->session->get("systemName"), "<a href='" . GIBBON_URL . "index.php?q=/modules/User Admin/preferences.php'>", "</a>"))) ;
				$body .= "<br/><br/>" ;
