...
class FacilityBookingGateway extends QueryableGateway
{
    ...
    public function queryFacilityBookings(QueryCriteria $criteria, $gibbonPersonID = null)
    {
        $query = $this
            ...
            ->cols([
                'gibbonTTSpaceBookingID', 'date', 'timeStart', 'timeEnd', 'gibbonSpace.name', 'gibbonPerson.preferredName', 'gibbonPerson.surname'
            ])
        ...
        $query->unionAll()
            ->from($this->getTableName())
            ->cols([
                'gibbonTTSpaceBookingID', 'date', 'timeStart', 'timeEnd', 'gibbonLibraryItem.name', 'gibbonPerson.preferredName', 'gibbonPerson.surname'
            ])
