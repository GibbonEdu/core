...
class CourseGateway extends QueryableGateway
{
    ...
    public function selectClassesByCourseID($gibbonCourseID)
    {
        ...
        $sql = "SELECT gibbonCourseClass.*, COUNT(CASE WHEN gibbonPerson.status='Full' AND (gibbonPerson.dateStart IS NULL OR gibbonPerson.dateStart<:today) THEN gibbonPerson.status END) as studentsActive, COUNT(CASE WHEN gibbonPerson.status='Expected' OR gibbonPerson.dateStart>=:today THEN gibbonPerson.status END) as studentsExpected, COUNT(DISTINCT gibbonPerson.gibbonPersonID) as studentsTotal
