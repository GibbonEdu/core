<div style='padding-top: 10px; font-size: 120px; font-weight: bold; font-family: arial, sans; text-align: center'>
    ...
    <?php if ($el->getField('gibbonPersonID') == $this->session->get('gibbonPersonID')) { ?>
        <p style='padding-right: 20px'><a style='color: #ff3' target='_parent' href='<?php echo GIBBON_URL; ?>index.php?q=/modules/System Admin/alarm_cancelProcess.php&divert=true&gibbonAlarmID=<?php echo $el->getField('gibbonAlarmID'); ?>'><?php echo $this->__('Turn Alarm Off'); ?></a></p> <?php
    } ?>
    </div>
    <?php if ($el->type == 'general') { ?>
        <?php echo $this->__('General Alarm!'); ?>
        <audio loop autoplay volume="3">
            <source src="./audio/alarm_general.mp3" type="audio/mpeg">
        </audio> <?php
    } elseif ($el->type == 'lockdown') {
        echo $this->__('Lockdown!'); ?>
        <audio loop autoplay volume="3">
