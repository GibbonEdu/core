    ...
    public function getNotificationTray($cacheLoad)
    {
        ...
        if ($this->session->has('username')) {
            ...
            if (isActionAccessible($guid, $connection2, '/modules/Messenger/messageWall_view.php')) {
                ...
                if (count($messages) < 1) {
                    $return .= "<a class='inactive inline-block relative mr-4' title='".__('Message Wall')."' href='$URL'><img class='minorLinkIcon' style='margin-left: 4px; opacity: 0.2; vertical-align: -75%' src='".$this->session->get('absoluteURL').'/themes/'.$this->session->get('gibbonThemeName')."/img/messageWall.png' alt='".__('Message Wall')."'></a>";
                } else {
