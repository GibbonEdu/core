...
class StaffDashboard implements OutputableInterface, ContainerAwareInterface
{
    ...
    protected function renderDashboard()
    {
        ...
        $attendanceAccess = isActionAccessible($guid, $connection2, Resource::fromRoute('Attendance', 'attendance_take_byFormGroup'));
        ...
        while ($rowFormGroups = $resultFormGroups->fetch()) {
            ...
            $behaviourView = isActionAccessible($guid, $connection2, Resource::fromRoute('Behaviour', 'behaviour_view'));
            if ($behaviourView) {
                //Behaviour
                $formGroups[$count][3] = '';
                $plural = 's';
                ...
                if (isActionAccessible($guid, $connection2, Resource::fromRoute('Behaviour', 'behaviour_manage_add'))) {
        ...
        if ($planner == false and $timetable == false and count($hooks) < 1) {
            ...
            if (isActionAccessible($guid, $connection2, Resource::fromRoute('Admissions', 'report_students_left')) || isActionAccessible($guid, $connection2, Resource::fromRoute('Admissions', 'report_students_new'))) {
