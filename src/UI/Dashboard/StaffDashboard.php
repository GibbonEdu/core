...
class StaffDashboard implements OutputableInterface
{
    ...
    protected function renderDashboard()
    {
        ...
        $date = date('Y-m-d');
        try {
            $data = array('gibbonSchoolYearID' => $session->get('gibbonSchoolYearID'), 'date' => $date, 'gibbonPersonID' => $session->get('gibbonPersonID'), 'gibbonSchoolYearID2' => $session->get('gibbonSchoolYearID'), 'date2' => $date, 'gibbonPersonID2' => $session->get('gibbonPersonID'));
        ...
        if ($result->rowCount() < 1) {
            ...
            $planner .= "<a href='".$session->get('absoluteURL')."/index.php?q=/modules/Planner/planner.php'>".__('View Planner').'</a>';
            $planner .= '</div>';
            ...
            while ($row = $result->fetch()) {
                if (!($row['role'] == 'Student' and $row['viewableStudents'] == 'N')) {
                    ...
                    $planner .= '</td>';
                    $planner .= '<td>';
                    $planner .= "<a href='".$session->get('absoluteURL').'/index.php?q=/modules/Planner/planner_view_full.php&viewBy=class&gibbonCourseClassID='.$row['gibbonCourseClassID'].'&gibbonPlannerEntryID='.$row['gibbonPlannerEntryID']."'><img title='".__('View')."' src='./themes/".$session->get('gibbonThemeName')."/img/plus.png'/></a>";
                    $planner .= '</td>';
                    $planner .= '</tr>';
                }
            }
        ...
        //GET TIMETABLE
        $timetable = false;
        if (isActionAccessible($guid, $connection2, '/modules/Timetable/tt.php') and $session->get('username') != '' and getRoleCategory($session->get('gibbonRoleIDCurrent'), $connection2) == 'Staff') {

            $timetable .= '
            <script type="text/javascript">
                $(document).ready(function(){
                    $("#tt").load("'.$session->get('absoluteURL').'/index_tt_ajax.php",{"gibbonTTID": "'.@$_GET['gibbonTTID'].'", "ttDate": "'.@$_POST['ttDate'].'", "fromTT": "'.@$_POST['fromTT'].'", "personalCalendar": "'.@$_POST['personalCalendar'].'", "schoolCalendar": "'.@$_POST['schoolCalendar'].'", "spaceBookingCalendar": "'.@$_POST['spaceBookingCalendar'].'"});
                });
            </script>   ';

            $timetable .= '<h2>'.__('My Timetable').'</h2>';
            $timetable .= "<div id='tt' name='tt' style='width: 100%; min-height: 40px; text-align: center'>";
            $timetable .= "<img style='margin: 10px 0 5px 0' src='".$session->get('absoluteURL')."/themes/Default/img/loading.gif' alt='".__('Loading')."' onclick='return false;' /><br/><p style='text-align: center'>".__('Loading').'</p>';
            $timetable .= '</div>';
        }
        ...
        $dataFormGroups = array('gibbonPersonIDTutor' => $session->get('gibbonPersonID'), 'gibbonPersonIDTutor2' => $session->get('gibbonPersonID'), 'gibbonPersonIDTutor3' => $session->get('gibbonPersonID'), 'gibbonSchoolYearID' => $session->get('gibbonSchoolYearID'));
        ...
        while ($rowFormGroups = $resultFormGroups->fetch()) {
            ...
            if ($behaviourView) {
                ...
                try {
                    $dataBehaviour = array('gibbonSchoolYearID' => $session->get('gibbonSchoolYearID'), 'gibbonSchoolYearID2' => $session->get('gibbonSchoolYearID'), 'gibbonFormGroupID' => $formGroups[$count][0]);
                ...
                if (isActionAccessible($guid, $connection2, '/modules/Behaviour/behaviour_manage_add.php')) {
                    ...
                    $formGroups[$count][3] .= "<a href='".$session->get('absoluteURL')."/index.php?q=/modules/Behaviour/behaviour_manage_add.php&gibbonPersonID=&gibbonFormGroupID=&gibbonYearGroupID=&type='>".__('Add')."<img style='margin: 0 0 -4px 5px' title='".__('Add')."' src='./themes/".$session->get('gibbonThemeName')."/img/page_new.png'/></a>";
                ...
                if ($resultBehaviour->rowCount() < 1) {
                    ...
                    while ($rowBehaviour = $resultBehaviour->fetch()) {
                        ...
                        if ($rowBehaviour['type'] == 'Negative') {
                            $formGroups[$count][3] .= "<img title='".__('Negative')."' src='./themes/".$session->get('gibbonThemeName')."/img/iconCross.png'/> ";
                        } elseif ($rowBehaviour['type'] == 'Positive') {
                            $formGroups[$count][3] .= "<img title='".__('Positive')."' src='./themes/".$session->get('gibbonThemeName')."/img/iconTick.png'/> ";
                        }
                        $formGroups[$count][3] .= '</td>';
                        $formGroups[$count][3] .= '<td>';
                        ...
                        if ($rowBehaviour['comment'] != '') {
                            $formGroups[$count][3] .= "<a title='".__('View Description')."' class='show_hide-$count2' onclick='false' href='#'><img style='padding-right: 5px' src='".$session->get('absoluteURL')."/themes/Default/img/page_down.png' alt='".__('Show Comment')."' onclick='return false;' /></a>";
                        }
                        $formGroups[$count][3] .= '</td>';
                        $formGroups[$count][3] .= '</tr>';
                        if ($rowBehaviour['comment'] != '') {
        ...
        if ($planner == false and $timetable == false and count($hooks) < 1) {
            ...
            foreach ($hooks as $hook) {
                ...
                $include = $session->get('absolutePath').'/modules/'.$hook['sourceModuleName'].'/'.$hook['sourceModuleInclude'];
