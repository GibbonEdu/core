...
class ParentDashboard implements OutputableInterface, ContainerAwareInterface
{
    ...
    protected function renderChildDashboard($gibbonPersonID, $dateStart)
    {
        ...
        $plannerOutput = "<span style='font-size: 85%; font-weight: bold'>".__('Today\'s Classes')."</span> . <span style='font-size: 70%'><a href='".$session->get('absoluteURL').'/index.php?q=/modules/Planner/planner.php&search='.$gibbonPersonID."'>".__('View Planner').'</a></span>';

        $classes = false;
        $date = date('Y-m-d');
        if (isSchoolOpen($guid, $date, $connection2) == true and isActionAccessible($guid, $connection2, '/modules/Planner/planner.php') and $session->get('username') != '') {
            try {
            ...
            if ($result->rowCount() > 0) {
                ...
                while ($row = $result->fetch()) {
                    ...
                    $plannerOutput .= '</td>';
                    $plannerOutput .= '<td>';
                    $plannerOutput .= "<a href='".$session->get('absoluteURL').'/index.php?q=/modules/Planner/planner_view_full.php&search='.$gibbonPersonID.'&viewBy=date&gibbonPlannerEntryID='.$row['gibbonPlannerEntryID']."&date=$date&width=1000&height=550'><img title='".__('View')."' src='./themes/".$session->get('gibbonThemeName')."/img/plus.png'/></a> ";
                    $plannerOutput .= '</td>';
                    $plannerOutput .= '</tr>';
                }
                $plannerOutput .= '</table>';
        ...
        $gradesOutput = "<div style='margin-top: 20px'><span style='font-size: 85%; font-weight: bold'>".__('Recent Feedback')."</span> . <span style='font-size: 70%'><a href='".$session->get('absoluteURL').'/index.php?q=/modules/Markbook/markbook_view.php&search='.$gibbonPersonID."'>".__('View Markbook').'</a></span></div>';
        $grades = false;
        ...
        try {
            $dataEntry = array('gibbonSchoolYearID' => $session->get('gibbonSchoolYearID'), 'gibbonPersonID' => $gibbonPersonID);
            ...
            while ($rowEntry = $resultEntry->fetch()) {
                ...
                if ($rowEntry['commentOn'] == 'N' and $rowEntry['uploadedResponseOn'] == 'N') {
                    ...
                    if ($rowEntry['response'] != '') {
                        $gradesOutput .= "<a title='".__('Uploaded Response')."' href='".$session->get('absoluteURL').'/'.$rowEntry['response']."'>".__('Uploaded Response').'</a><br/>';
                    }
                    $gradesOutput .= '</td>';
                }
                    ...
                    if ($resultSub->rowCount() != 1) {
                        ...
                        if ($resultWork->rowCount() > 0) {
                            ...
                            if ($rowWork['type'] == 'File') {
                                $gradesOutput .= "<span title='".$rowWork['version'].". $status. ".sprintf(__('Submitted at %1$s on %2$s'), substr($rowWork['timestamp'], 11, 5), Format::date(substr($rowWork['timestamp'], 0, 10)))."' $style><a href='".$session->get('absoluteURL').'/'.$rowWork['location']."'>$linkText</a></span>";
        ...
        $deadlinesOutput = "<div style='margin-top: 20px'><span style='font-size: 85%; font-weight: bold'>".__('Upcoming Due Dates')."</span> . <span style='font-size: 70%'><a href='".$session->get('absoluteURL').'/index.php?q=/modules/Planner/planner_deadlines.php&search='.$gibbonPersonID."'>".__('View {homeworkName}', ['homeworkName' => __($homeworkNamePlural)]).'</a></span></div>';
        $deadlines = false;
        ...
        if (!(isActionAccessible($guid, $connection2, '/modules/Activities/activities_view.php'))) {
            ...
            $activitiesOutput .= "<a href='".$session->get('absoluteURL')."/index.php?q=/modules/Activities/activities_view.php&gibbonPersonID=".$gibbonPersonID."'>".__('View Available Activities').'</a>';
            $activitiesOutput .= '</div>';
                ...
                while ($rowYears = $resultYears->fetch()) {
                    ...
                    if ($result->rowCount() < 1) {
                        ...
                        while ($row = $result->fetch()) {
                            ...
                            $activitiesOutput .= '<td>';
                            if ($dateType != 'Date') {
                                $terms = getTerms($connection2, $session->get('gibbonSchoolYearID'), true);
                                $termList = '';
