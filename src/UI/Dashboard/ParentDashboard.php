...
class ParentDashboard implements OutputableInterface, ContainerAwareInterface
{
    ...
    protected function renderChildDashboard($gibbonPersonID, $dateStart)
    {
        ...
        if (isActionAccessible($guid, $connection2, Action::fromRoute('Planner', 'planner'))) {
            ...
            $date = date('Y-m-d');
            if (isSchoolOpen($guid, $date, $connection2) == true and isActionAccessible($guid, $connection2, Action::fromRoute('Planner', 'planner')) and $this->session->get('username') != '') {
                try {
        ...
        if (!(isActionAccessible($guid, $connection2, Action::fromRoute('Activities', 'activities_view')))) {
