...
if (isActionAccessible($guid, $connection2, "/modules/System Admin/theme_manage.php")==FALSE) {
	...
	print "<div class='trailHead'><a href='" . $_SESSION[$guid]["absoluteURL"] . "'>Home</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . getModuleName($_GET["q"]) . "/" . getModuleEntry($_GET["q"], $connection2, $guid) . "'>" . getModuleName($_GET["q"]) . "</a> > </div><div class='trailEnd'>Manage Themes</div>" ;
	print "</div>" ;
	...
	if (!($updateReturn=="")) {
		if ($updateReturn=="fail0") {
			...
			$updateReturnMessage ="Update failed because all themes could not be deactivated." ;	
		}
