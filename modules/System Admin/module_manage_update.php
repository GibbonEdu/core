...
if (isActionAccessible($guid, $connection2, '/modules/System Admin/module_manage_update.php') == false) {
    ...
    if ($gibbonModuleID == '') {
        ...
        if ($result->rowCount() != 1) {
            ...
            echo '<p>';
            echo sprintf(__('This page allows you to semi-automatically update the %1$s module to a new version. You need to take care of the file updates, and based on the new files, Gibbon will do the database upgrades.'), htmlPrep($values['name']));
            echo '</p>';
