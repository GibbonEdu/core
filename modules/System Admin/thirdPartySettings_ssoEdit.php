...
if (isActionAccessible($guid, $connection2, '/modules/System Admin/thirdPartySettings.php') == false) {
    ...
    $setting = $settingGateway->getSettingByScope('System Admin', 'sso'.$sso);
    ...
    if ($sso == 'Google') {
        ...
        $row = $form->addRow()->addClass('googleActive');
            ...
            $row->addTextArea('clientSecret')->required();
            
        $row = $form->addRow()->addClass('googleActive');
            $row->addLabel('redirectUri', __('Google Developers Redirect Url'))->description(__('Google Redirect on sucessful auth.'));
            $row->addTextArea('redirectUri')->required();
            
        $row = $form->addRow()->addClass('googleActive');
            $row->addLabel('developerKey', __('Google Developers Developer Key'))->description(__('Google project Developer Key.'));
            $row->addTextArea('developerKey')->required();
            
        $row = $form->addRow()->addClass('googleActive');
            $row->addLabel('calendarFeed', __('School Google Calendar ID'))->description(__('Google Calendar ID for your school calendar. Only enables timetable integration when logging in via Google.'));
            $row->addTextField('calendarFeed');
        ...
        $form->addRow()->addHeading(__('Generic OAuth2 Provider'))->append(__('If your school uses a service that offers OAuth2 authentication, you can enable single sign on integration with Gibbon. This process makes use of industry-standard OAuth2 protocols, and allows a user to access Gibbon without a username and password, provided that their listed email address is part of the chosen service and is unique in Gibbon. This setting offers a generic implementation of OAuth2, if your service requires specific parameters, it will likely not work.'));
    ...
    if ($sso == 'Microsoft' || $sso == 'Other') {
        ...
        $row = $form->addRow()->addClass('settingActive');
            $row->addLabel('redirectUri', __('API Redirect Url'))->description(__('Where to redirect to on successful authentication'));
            $row->addTextArea('redirectUri')->required();
    }
