...
if (isActionAccessible($guid, $connection2, "/modules/System Admin/theme_manage_uninstall.php")==FALSE) {
	...
	print "<div class='trailHead'><a href='" . $_SESSION[$guid]["absoluteURL"] . "'>Home</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . getModuleName($_GET["q"]) . "/" . getModuleEntry($_GET["q"], $connection2, $guid) . "'>" . getModuleName($_GET["q"]) . "</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . getModuleName($_GET["q"]) . "/theme_manage.php'>Manage Themes</a> > </div><div class='trailEnd'>Uninstall Theme</div>" ;
	print "</div>" ;
	...
	if (!($deleteReturn=="")) {
		if ($deleteReturn=="fail0") {
			...
			$deleteReturnMessage ="Uninstall failed because module does not exist or is active." ;	
		}
	...
	if ($gibbonThemeID=="") {
		...
		if ($result->rowCount()!=1) {
			...
			$row=$result->fetch() ;
			?>
			<form method="post" action="<? print $_SESSION[$guid]["absoluteURL"] . "/modules/" . $_SESSION[$guid]["module"] . "/theme_manage_uninstallProcess.php?gibbonThemeID=$gibbonThemeID" ?>">
