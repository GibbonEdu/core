...
if (isActionAccessible($guid, $connection2, '/modules/System Admin/emailTemplates_manage_edit.php') == false) {
    ...
    $form->addRow()->addHeading(__('Email Template'))->prepend(Format::link('https://twig.symfony.com/doc/2.x/', '<img class="float-right w-5 h-5" title="'.__('Twig Documentation').'"  src="./themes/Default/img/help.png" >'));

    $variables = json_decode($values['variables'] ?? '');
    ...
    $row = $form->addRow();
        ...
        $column->addContent(implode(', ', $variables));
