...
if (isActionAccessible($guid, $connection2, '/modules/Students/report_students_byRollGroup.php') == false) {
    ...
    $table->addColumn('username', __('Id.'))->width('5%');
    ...
    if ($criteria->hasFilter('view', 'extended')) {
        ...
        $table->addColumn('religion', __('Religion')); //GS
        # Guardian 1
        $table->addColumn('guardian1', __('Guardian 1'))
            ->sortable(['g1surname', 'g1name'])
            ->format(function ($person) {
            return Format::name('', $person['g1name'], $person['g1surname'], 'Parent', true, true) . '<br/><small><i>'.Format::userStatusInfo($person).'</i></small>';
        });
        $table->addColumn('g1phone', __('G1 Phone'));
        $table->addColumn('g1profession', __('G1 Profession'));
        $table->addColumn('g1salary', __('G1 Salary'));
/*        # Guardian 2
        $table->addColumn('guardian2', __('Guardian 1'))
            ->sortable(['guardian2surname', 'guardian2name'])
            ->format(function ($person) {
            return Format::name('', $person['g2name'], $person['g2surname'], 'Parent', true, true) . '<br/><small><i>'.Format::userStatusInfo($person).'</i></small>';
        });
        $table->addColumn('g2phone', __('G2 Phone'));
        $table->addColumn('g2profession', __('G2 Profession'));
        $table->addColumn('g2salary', __('G2 Salary'));
*/
