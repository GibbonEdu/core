	...
	if (count($externalAssessmentDataPoints)==0 AND count($internalAssessmentDataPoints)==0) { //Seems like things are not configured, so give appropriate information according to access
		...
		print "<p>" ;
			if (isActionAccessible($guid, $connection2, "/modules/School Admin/trackingSettings.php")==FALSE) { //No access, just give warning
				...
				print sprintf(_('Data Points has been configured to export certain key assessment data. Please take a look at %1$sTracking Settings%2$s to change what data points are included.'), "<a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/School Admin/trackingSettings.php'>", "</a>") . ". Use the export button below to prepare your Data Points export for download.<br/>" ;
			}
			print "<br/>" ;
		print "</p>" ;
		print "<div class='warning'>" ;
			print _("Warning, please note that this process is resource intensive, and may slow down access to the system for other users. Please be patient as the download might take a few minutes to prepare.") ;
			print "<div class='linkTop'>" ;
				print "<a href='" . $_SESSION[$guid]["absoluteURL"] . "/modules/" . $_SESSION[$guid]["module"] . "/dataPoints_contents.php'>" . _("Export to Excel") . " <img title='" . _('Export to Excel') . "' src='./themes/" . $_SESSION[$guid]["gibbonThemeName"] . "/img/download.png'/></a>" ;
			print "</div>" ;
		print "</div>" ;
	}
