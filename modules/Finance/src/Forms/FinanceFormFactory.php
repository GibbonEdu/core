...
class FinanceFormFactory extends DatabaseFormFactory
{
    ...
    //Van Hoa 
    public function createSelectTeacher($name)
    {
        $sql = "
            SELECT 
                CONCAT(gibbonPerson.gibbonPersonID, '|', gibbonFormGroup.gibbonFormGroupID) AS value, 
                CONCAT(gibbonPerson.surname, ' ', gibbonPerson.preferredName, ' - ', gibbonFormGroup.name) AS name
            FROM gibbonFormGroup
            JOIN gibbonPerson ON gibbonFormGroup.gibbonPersonIDTutor = gibbonPerson.gibbonPersonID
            WHERE gibbonFormGroup.gibbonPersonIDTutor IS NOT NULL
            ORDER BY gibbonPerson.surname, gibbonPerson.preferredName, gibbonFormGroup.name
        ";
    
        return $this->createSelect($name)
            ->fromQuery($this->pdo, $sql)
            ->placeholder(__('Chọn giáo viên và lớp'));
    }
    //Van Hoa
