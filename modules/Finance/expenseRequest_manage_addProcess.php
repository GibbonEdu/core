...
if ($gibbonFinanceBudgetCycleID=="") {
	...
	$URL=$_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . getModuleName($_POST["address"]) . "/expenseRequest_manage_add.php&gibbonFinanceBudgetCycleID=$gibbonFinanceBudgetCycleID&status=$status&gibbonFinanceInvoiceeID=$gibbonFinanceInvoiceeID&monthOfIssue=$monthOfIssue&gibbonFinanceBillingScheduleID=$gibbonFinanceBillingScheduleID" ;
				...
				foreach ($gibbonFinanceInvoiceeIDs AS $gibbonFinanceInvoiceeID) {
					...
					if ($thisStudentFailed==FALSE) {
						...
						if ($invoiceTo=="Family" OR $companyFamily==TRUE) {
							...
							try {
								if ($scheduling=="Scheduled") {
									$dataInvoice=array("gibbonFinanceBudgetCycleID"=>$gibbonFinanceBudgetCycleID, "gibbonFinanceInvoiceeID"=>$gibbonFinanceInvoiceeID, "gibbonFinanceBillingScheduleID"=>$gibbonFinanceBillingScheduleID); 
									$sqlInvoice="SELECT * FROM gibbonFinanceInvoice WHERE gibbonFinanceBudgetCycleID=:gibbonFinanceBudgetCycleID AND gibbonFinanceInvoiceeID=:gibbonFinanceInvoiceeID AND invoiceTo='Family' AND billingScheduleType='Scheduled' AND gibbonFinanceBillingScheduleID=:gibbonFinanceBillingScheduleID AND status='Pending'" ;
								}
								else {
									$dataInvoice=array("gibbonFinanceBudgetCycleID"=>$gibbonFinanceBudgetCycleID, "gibbonFinanceInvoiceeID"=>$gibbonFinanceInvoiceeID); 
									$sqlInvoice="SELECT * FROM gibbonFinanceInvoice WHERE gibbonFinanceBudgetCycleID=:gibbonFinanceBudgetCycleID AND gibbonFinanceInvoiceeID=:gibbonFinanceInvoiceeID AND invoiceTo='Family' AND billingScheduleType='Ad Hoc' AND status='Pending'" ;
								}
							...
							if ($resultInvoice->rowCount()==0 AND $thisInvoiceFailed==FALSE) {
								...
								if ($AI=="") {
									...
									if ($continue==FALSE) {
										...
										try {
											if ($scheduling=="Scheduled") {
												$dataInvoiceAdd=array("gibbonFinanceBudgetCycleID"=>$gibbonFinanceBudgetCycleID, "gibbonFinanceInvoiceeID"=>$gibbonFinanceInvoiceeID, "gibbonFinanceBillingScheduleID"=>$gibbonFinanceBillingScheduleID, "notes"=>$notes, "key"=>$key, "gibbonPersonIDCreator"=>$_SESSION[$guid]["gibbonPersonID"]); 
												$sqlInvoiceAdd="INSERT INTO gibbonFinanceInvoice SET gibbonFinanceBudgetCycleID=:gibbonFinanceBudgetCycleID, gibbonFinanceInvoiceeID=:gibbonFinanceInvoiceeID, invoiceTo='Family', billingScheduleType='Scheduled', gibbonFinanceBillingScheduleID=:gibbonFinanceBillingScheduleID, notes=:notes, `key`=:key, status='Pending', separated='N', gibbonPersonIDCreator=:gibbonPersonIDCreator, timeStampCreator='" . date("Y-m-d H:i:s") . "'" ;
											}
											else {
												$dataInvoiceAdd=array("gibbonFinanceBudgetCycleID"=>$gibbonFinanceBudgetCycleID, "gibbonFinanceInvoiceeID"=>$gibbonFinanceInvoiceeID, "invoiceDueDate"=>dateConvert($guid, $invoiceDueDate), "notes"=>$notes, "key"=>$key, "gibbonPersonIDCreator"=>$_SESSION[$guid]["gibbonPersonID"]); 
						...
						if (($invoiceTo=="Company" AND $companyAll=="Y" ) OR ($invoiceTo=="Company" AND $companyAll=="N" AND $companyFamilyCompanyHasCharges==TRUE)) {
							...
							try {
								if ($scheduling=="Scheduled") {
									$dataInvoice=array("gibbonFinanceBudgetCycleID"=>$gibbonFinanceBudgetCycleID, "gibbonFinanceInvoiceeID"=>$gibbonFinanceInvoiceeID, "gibbonFinanceBillingScheduleID"=>$gibbonFinanceBillingScheduleID); 
									$sqlInvoice="SELECT * FROM gibbonFinanceInvoice WHERE gibbonFinanceBudgetCycleID=:gibbonFinanceBudgetCycleID AND gibbonFinanceInvoiceeID=:gibbonFinanceInvoiceeID AND invoiceTo='Company' AND billingScheduleType='Scheduled' AND gibbonFinanceBillingScheduleID=:gibbonFinanceBillingScheduleID AND status='Pending'" ;
								}
								else {
									$dataInvoice=array("gibbonFinanceBudgetCycleID"=>$gibbonFinanceBudgetCycleID, "gibbonFinanceInvoiceeID"=>$gibbonFinanceInvoiceeID); 
									$sqlInvoice="SELECT * FROM gibbonFinanceInvoice WHERE gibbonFinanceBudgetCycleID=:gibbonFinanceBudgetCycleID AND gibbonFinanceInvoiceeID=:gibbonFinanceInvoiceeID AND invoiceTo='Company' AND billingScheduleType='Ad Hoc' AND status='Pending'" ;
								}
								$resultInvoice=$connection2->prepare($sqlInvoice);
								$resultInvoice->execute($dataInvoice);
							}
