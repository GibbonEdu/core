...
if (isActionAccessible($guid, $connection2, '/modules/Finance/feeCategories_manage_delete.php') == false) {
    ...
    if ($gibbonFinanceFeeCategoryID == '') {
        ...
        if ($result->rowCount() != 1) {
            ...
            $row = $result->fetch()
            ?>
			<form method="post" action="<?php echo $_SESSION[$guid]['absoluteURL'].'/modules/'.$_SESSION[$guid]['module']."/feeCategories_manage_deleteProcess.php?gibbonFinanceFeeCategoryID=$gibbonFinanceFeeCategoryID" ?>">
            ...
            ?></span>
					...
					<tr>
						<td> 
							<input name="gibbonFinanceFeeCategoryID" id="gibbonFinanceFeeCategoryID" value="<?php echo $gibbonFinanceFeeCategoryID ?>" type="hidden">
