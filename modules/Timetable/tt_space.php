...
if (isActionAccessible($guid, $connection2, "/modules/Timetable/tt_space.php")==FALSE) {
	...
	if ($highestAction==FALSE) {
		...
		print "<div class='trailHead'><a href='" . $_SESSION[$guid]["absoluteURL"] . "'>" . _("Home") . "</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . _(getModuleName($_GET["q"])) . "/" . getModuleEntry($_GET["q"], $connection2, $guid) . "'>" . getModuleName($_GET["q"]) . "</a> > </div><div class='trailEnd'>" . _('View Timetable by Space') . "</div>" ;
		print "</div>" ;
