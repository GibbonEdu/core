...
function renderTT($guid, $connection2, $gibbonPersonID, $gibbonTTID, $title = '', $startDayStamp = '', $q = '', $params = '', $narrow = 'full', $edit = false)
{
    ...
    if ($proceed == false) {
        ...
        if ($result->rowCount() > 0) {
            ...
            $spaceBookingAvailable = isActionAccessible($guid, $connection2, Action::fromRoute('Timetable', 'spaceBooking_manage'));
            ...
            $canViewAbsences = isActionAccessible($guid, $connection2, Action::fromRoute('Staff', 'absences_view_byPerson'));
            ...
            while ($rowPeriods = $resultPeriods->fetch()) {
                ...
                if ($isSlotInTime == true) {
                    ...
                    if ($resultException->rowCount() < 1) {
                        ...
                        if (isActionAccessible($guid, $connection2, Action::fromRoute('Departments', 'department_course_class')) and $edit == false) {
                        ...
                        } elseif (isActionAccessible($guid, $connection2, Action::fromRoute('Timetable Admin', 'courseEnrolment_manage_class_edit')) and $edit == true) {
...
function renderTTSpaceDay($guid, $connection2, $gibbonTTID, $startDayStamp, $count, $daysInWeek, $gibbonSpaceID, $gridTimeStart, $diffTime, $eventsSpaceBooking, $specialDayStart = '', $specialDayEnd = '')
{
    ...
    $canAddBookings = isActionAccessible($guid, $connection2, Action::fromRoute('Timetable', 'spaceBooking_manage_add'));
    $canAddChanges = isActionAccessible($guid, $connection2, Action::fromRoute('Timetable', 'spaceChange_manage_add'));
    $canEditTTDays = isActionAccessible($guid, $connection2, Action::fromRoute('Timetable Admin', 'tt_edit_day_edit_class_edit'));
