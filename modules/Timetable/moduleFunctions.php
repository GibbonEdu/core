...
function getCalendarEvents($connection2, $guid, $xml, $startDayStamp, $endDayStamp)
{
    ...
    if ($googleOAuth == 'Y' and isset($session->get('googleAPIAccessToken'))) {
        $eventsSchool = array();
    ...
    if ($proceed == false) {
        ...
        if ($gibbonPersonID == $session->get('gibbonPersonID') and $edit == false) {
            ...
            if (!empty($_POST['fromTT']) && $_POST['fromTT'] == 'Y') {
                $session->get('viewCalendarSchool') = !empty($_POST['schoolCalendar']) ? 'Y' : 'N';
                $session->get('viewCalendarPersonal') = !empty($_POST['personalCalendar']) ? 'Y' : 'N';
                $session->get('viewCalendarSpaceBooking') = !empty($_POST['spaceBookingCalendar']) ? 'Y' : 'N';
            }
        ...
        if ($result->rowCount() > 0) {
            ...
            $output .= ";'>";
                ...
                if ($self == true and ($session->get('calendarFeed') != '' or $session->get('calendarFeedPersonal') != '' or $session->get('viewCalendarSpaceBooking') != '')) {
                    ...
                    if ($session->get('calendarFeedPersonal') != '' and isset($session->get('googleAPIAccessToken'))) {
                        $checked = '';
