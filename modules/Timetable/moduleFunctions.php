...
function renderTT($guid, $connection2, $gibbonPersonID, $gibbonTTID, $title = '', $startDayStamp = '', $q = '', $params = '', $narrow = 'full', $edit = false)
{
    ...
    if ($proceed == false) {
        ...
        if ($result->rowCount() > 0) {
            ...
            $spaceBookingAvailable = isActionAccessible($guid, $connection2, Resource::fromRoute('Timetable', 'spaceBooking_manage'));
            ...
            $canViewAbsences = isActionAccessible($guid, $connection2, Resource::fromRoute('Staff', 'absences_view_byPerson'));
            ...
            foreach ($eventsSchool as $event) {
                if (date('Y-m-d', $event[2]) == date('Y-m-d', ($startDayStamp + (86400 * $count)))) {
                    if ($event[1] == 'All Day') {
                        ...
                        $output .= "<div class='ttSchoolCalendar' $title style='z-index: $zCount; position: absolute; top: $top; width:100%; min-width: $width ; border: 1px solid #555; height: {$height}px; margin: 0px; padding: 0px; opacity: $schoolCalendarAlpha'>";
            ...
            foreach ($eventsPersonal as $event) {
                if (date('Y-m-d', $event[2]) == date('Y-m-d', ($startDayStamp + (86400 * $count)))) {
                    if ($event[1] == 'All Day') {
                        ...
                        $height = 30;
                        $top = (($maxAllDays * -31) - 8 + ($allDay * 30)).'px';
                        $output .= "<div class='ttPersonalCalendar' $title style='z-index: $zCount; position: absolute; top: $top; width:100%; min-width: $width ; border: 1px solid #555; height: {$height}px; margin: 0px; padding: 0px; opacity: $schoolCalendarAlpha'>";
            ...
            while ($rowPeriods = $resultPeriods->fetch()) {
                ...
                if ($isSlotInTime == true) {
                    ...
                    if ($resultException->rowCount() < 1) {
                        ...
                        if (isActionAccessible($guid, $connection2, Resource::fromRoute('Departments', 'department_course_class')) and $edit == false) {
                        ...
                        } elseif (isActionAccessible($guid, $connection2, Resource::fromRoute('Timetable Admin', 'courseEnrolment_manage_class_edit')) and $edit == true) {
...
function renderTTSpaceDay($guid, $connection2, $gibbonTTID, $startDayStamp, $count, $daysInWeek, $gibbonSpaceID, $gridTimeStart, $diffTime, $eventsSpaceBooking, $specialDayStart = '', $specialDayEnd = '')
{
    ...
    $canAddBookings = isActionAccessible($guid, $connection2, Resource::fromRoute('Timetable', 'spaceBooking_manage_add'));
    $canAddChanges = isActionAccessible($guid, $connection2, Resource::fromRoute('Timetable', 'spaceChange_manage_add'));
    $canEditTTDays = isActionAccessible($guid, $connection2, Resource::fromRoute('Timetable Admin', 'tt_edit_day_edit_class_edit'));
            ...
            if ($isSlotInTime == true) {
                if ((isset($spaceChanges[str_pad($rowPeriods['gibbonTTDayRowClassID'], 12, '0', STR_PAD_LEFT)]) == false and $rowPeriods['type'] == 'Normal') or $rowPeriods['type'] == 'Change') {
                    ...
                    if ((date('H:i:s') > $effectiveStart) and (date('H:i:s') < $effectiveEnd) and $date == date('Y-m-d')) {
                        ...
                        //Create div to represent period
                        $output .= "<div class='$class2' $title style='z-index: $zCount; position: absolute; top: $top; width: 100%; min-width: $width; height: {$height}px; margin: 0px; padding: 0px; opacity: $ttAlpha'>";
                    if ($height >= 45) {
                        $output .= $rowPeriods['name'].'<br/>';
                        $output .= '<i>'.substr($effectiveStart, 0, 5).' - '.substr($effectiveEnd, 0, 5).'</i><br/>';
                    }
