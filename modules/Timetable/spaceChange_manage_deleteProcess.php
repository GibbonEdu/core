...
if (isActionAccessible($guid, $connection2, Resource::fromRoute('Timetable', 'spaceChange_manage_delete')) == false) {
    $URL .= '&return=error0';
    header("Location: {$URL}");
} else {
            ...
            if ($resultSelect->rowCount() != 1) {
                ...
                try {
                    if ($highestAction == 'Manage Facility Changes_allClasses' OR $highestAction == 'Manage Facility Changes_myDepartment') {
                        ...
                        $data = array('gibbonPersonID' => $session->get('gibbonPersonID'), 'gibbonTTSpaceChangeID' => $gibbonTTSpaceChangeID);
