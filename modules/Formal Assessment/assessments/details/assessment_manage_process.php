...
if (isActionAccessible($guid, $connection2, '/modules/Formal Assessment/assessments/details/assessment_manage_process.php') == false) {
    // Access denied
    ...
    if ($gibbonCourseClassID == '') {
        ...
        if ($result->rowCount() != 1) {
            ...
            $page->breadcrumbs
                ->add(__('Manage {courseClass} Assessments', ['courseClass' => $row['course'].'.'.$row['class']), 'assessment_manage.php', ['gibbonCourseClassID' => $gibbonCourseClassID, 'type' => $type])
                ->add(__('Process Assessment'));

            $page->return->addReturns(['error3' => __('Your request failed due to an attachment error.')]);

            $form = Form::create('assessment', $session->get('absoluteURL').'/modules/'.$session->get('module').'/assessments/details/assessment_manage_process.php?gibbonCourseClassID='.$gibbonCourseClassID.'&type='.$type.'&address='.$session->get('address'));
            ...
            $types = $settingGateway->getSettingByScope('Formal Assessment', 'assessmentTypes');
            if (!empty($types)) {
                $row = $form->addRow();
