...
function externalAssessmentDetails($guid, $gibbonPersonID, $connection2, $gibbonYearGroupID = null, $manage = false, $search = '', $allStudents = '')
{
    ...
    if ($resultAssessments->rowCount() < 1) {
        ...
        // Formatter for abbrivated month name, based on locale.
        $monthNameFormatter = new \IntlDateFormatter(null);
        $monthNameFormatter->setPattern("MMM");
        while ($rowAssessments = $resultAssessments->fetch()) {
            echo '<h2>';
            echo __($rowAssessments['name'])." <span style='font-size: 75%; font-style: italic'>(".$monthNameFormatter->format(mktime(0, 0, 0, substr($rowAssessments['date'], 5, 2))).' '.substr($rowAssessments['date'], 0, 4).')</span>';
            if ($manage == true) {
