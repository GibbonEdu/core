...
if (isActionAccessible($guid, $connection2, '/modules/Formal Assessment/import_internal_results.php') == false) {
    ...
    if ($step == 1) {
        ...
        echo '</h2>';
        echo '<p>';
        echo __('This page allows you to import internal assessment results from a CSV file into existing assessment columns. The system will match assessments by course name, class name and column name, updating any matching results, whilst creating new results not already existing in the system.');
        echo '</p>';

        $form = Form::create('importInternalResults', $_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module'].'/import_internal_results.php&step=2');
