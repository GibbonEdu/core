...
function classChooser($guid, $pdo, $gibbonCourseClassID)
{
    ...
    if ($enableGroupByTerm == 'Y' ) {
        $selectTerm = (isset($session->get('markbookTerm')))? $session->get('markbookTerm') : 0;
        ...
        $session->get('markbookTermName') = isset($terms[$selectTerm])? $terms[$selectTerm] : $selectTerm;
        $session->get('markbookTerm') = $selectTerm;
    } else {
        $session->get('markbookTerm') = 0;
        $session->get('markbookTermName') = __('All Columns');
    }
    ...
    if ($rollOrderCount > 0) {
        $selectOrderBy = (isset($session->get('markbookOrderBy')))? $session->get('markbookOrderBy') : 'surname';
        ...
        $session->get('markbookOrderBy') = $selectOrderBy;
    }

    // SHOW
    $selectFilter = (isset($session->get('markbookFilter')))? $session->get('markbookFilter') : '';
    $selectFilter = (isset($_GET['markbookFilter']))? $_GET['markbookFilter'] : $selectFilter;

    $session->get('markbookFilter') = $selectFilter;
