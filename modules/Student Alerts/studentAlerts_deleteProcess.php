...
$URL = $session->get('absoluteURL').'/index.php?q=/modules/Student Alerts/studentAlerts_manage.php';

if (isActionAccessible($guid, $connection2, '/modules/Student Alerts/studentAlerts_delete.php') == false) {
    $URL .= '&return=error0';
    header("Location: {$URL}");
} elseif (empty($gibbonAlertID)) {
    $URL .= '&return=error1';
    header("Location: {$URL}");
    exit;
} else {
    ...
    $deleted = $alertGateway->delete($gibbonAlertID);
