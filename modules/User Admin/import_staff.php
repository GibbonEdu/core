...
if (isActionAccessible($guid, $connection2, '/modules/User Admin/import_staff.php') == false) {
    ...
    //Proceed!
    $page->breadcrumbs->add(__('Import Staff'));        
        ...
        <ol>
            ...
            <li><?php echo __('The submitted file must have the following fields in the following order (* denotes required field):') ?></li>
                <ol>
                    ...
                    <li><b><?php echo __('Type') ?> *</b> - <?php echo __('Teaching or Support') ?></li>
                    <li><b><?php echo __('Initials') ?></b> - <?php echo __('Must be unique if set.') ?></li>
                    <li><b><?php echo __('Job Title') ?></b></li>
                </ol>
            </li>
                ...
                if ($lockFail == false) {
                    ...
                    if ($proceed == true) {
                        ...
                        while (($data = fgetcsv($handle, 100000, stripslashes($_POST['fieldDelimiter']), stripslashes($_POST['stringEnclosure']))) !== false) {
                            if ($data[1] != '' and $data[2] != '') {
                                ...
                                echo "<div class='error'>";
                                echo sprintf(__('Staff with username %1$s had some information malformations.'), $data[7]);
                                echo '</div>';
                            }
                            ++$userCount;
                        }
                        ...
                        if ($userSuccessCount == 0) {
                            echo "<div class='error'>";
                            echo __('No useful staff were detected in the import file (perhaps they did not meet minimum requirements), so the import will be aborted.');
                            echo '</div>';
                            $proceed = false;
                        } elseif ($userSuccessCount < $userCount) {
                            echo "<div class='error'>";
                            echo __('Some staff could not be successfully read or used, so the import will be aborted.');
                            echo '</div>';
                            $proceed = false;
                        } elseif ($userSuccessCount == $userCount) {
                            echo "<div class='success'>";
                            echo __('All staff could be read and used, so the import will proceed.');
                            echo '</div>';
                        } else {
                            echo "<div class='error'>";
                    ...
                    if ($proceed == true) {
                        ...
                        if ($deleteAllFail == true) {
                            echo "<div class='error'>";
                            echo __('An error was encountered in deleting all staff.');
                            echo '</div>';
                        } else {
                            echo "<div class='success'>";
                            echo __('All staff were deleted.');
                            echo '</div>';
                        }

                        if ($deleteAllFail == false) {
                            echo '<h4>';
                            echo __('Import All Staff');
                            echo '</h4>';
                            foreach ($users as $user) {
                                ...
                                if ($addUserFail == true) {
                                    echo "<div class='error'>";
                                    echo __('There was an error adding staff:').' '.$user['username'].'.';
                                    echo '</div>';
                                } else {
                                    echo "<div class='success'>";
                                    echo sprintf(__('User %1$s was successfully added.'), $user['username']);
                                    echo '</div>';
                                }
                            }
                        }
