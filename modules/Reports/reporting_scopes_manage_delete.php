...
if (isActionAccessible($guid, $connection2, '/modules/Reports/reporting_scopes_manage_delete.php') == false) {
    ...
    if (empty($gibbonReportingScopeID) || empty($gibbonReportingCycleID)) {
        $page->addError(__('You have not specified one or more required parameters.'));
        return;
    }

    $values = $container->get(ReportingScopeGateway::class)->getByID($gibbonReportingScopeID);
    ...
    $form = DeleteForm::createForm($session->get('absoluteURL').'/modules/Reports/reporting_scopes_manage_deleteProcess.php', true);
