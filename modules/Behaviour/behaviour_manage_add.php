								...
								<select name="gibbonPlannerEntryID" id="gibbonPlannerEntryID" style="width: 302px">
									...
									while ($rowSelect=$resultSelect->fetch()) {
										...
										if ($show==TRUE) {
											...
											print "<option value='" . $rowSelect["gibbonPlannerEntryID"] . "'>" . htmlPrep($rowSelect["course"]) . "." . htmlPrep($rowSelect["class"]) . " " . htmlPrep($rowSelect["lesson"]) . " - " . substr(dateConvertBack($rowSelect["date"]),0,5) . "$submission</option>" ;
										}
									}
									?>			
								</select>
