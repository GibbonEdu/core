...
if (isActionAccessible($guid, $connection2, "/modules/Planner/outcomes_delete.php")==FALSE) {
	...
	if ($highestAction==FALSE) {
		...
		if ($highestAction!="Manage Outcomes_viewEditAll" AND $highestAction!="Manage Outcomes_viewAllEditLearningArea") {
			...
			print "<div class='trailHead'><a href='" . $_SESSION[$guid]["absoluteURL"] . "'>" . _("Home") . "</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . getModuleName($_GET["q"]) . "/" . getModuleEntry($_GET["q"], $connection2, $guid) . "'>" . getModuleName($_GET["q"]) . "</a> > </div><div class='trailEnd'>Delete Item</div>" ;
			print "</div>" ;
			...
			if ($gibbonOutcomeID=="") {
				...
				if ($result->rowCount()!=1) {
					...
					$row=$result->fetch() 
					?>
					<form method="post" action="<?php print $_SESSION[$guid]["absoluteURL"] . "/modules/" . $_SESSION[$guid]["module"] . "/outcomes_deleteProcess.php?gibbonOutcomeID=$gibbonOutcomeID" ?>">
