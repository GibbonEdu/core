            ...
            if ($result->rowCount() != 1) {
                ...
                if ($gibbonUnitID == '') {
                    ...
                    if ($result->rowCount() != 1) {
                        ...
                        echo sprintf(__($guid, 'This function allows you to take all of the lesson content (Details and Teacher\'s Notes) from the selected working unit (%1$s in %2$s) and use them to create new Smart Blocks in the master unit, which are then used to replace the original content in the working unit. In this way you can quickl "smart blockify" an existing unit.'), $row['name'], "$course.$class");
                        echo '</p>';
