...
if (isActionAccessible($guid, $connection2, "/modules/School Admin/studentsSettings_noteCategory_delete.php")==FALSE) {
	...
	if ($gibbonStudentNoteCategoryID=="") {
		...
		if ($result->rowCount()!=1) {
			...
			$row=$result->fetch() ;
			?>
			<form method="post" action="<? print $_SESSION[$guid]["absoluteURL"] . "/modules/" . $_SESSION[$guid]["module"] . "/studentsSettings_noteCategory_deleteProcess.php?gibbonStudentNoteCategoryID=$gibbonStudentNoteCategoryID" ?>">
