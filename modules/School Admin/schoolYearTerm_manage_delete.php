...
if (isActionAccessible($guid, $connection2, "/modules/School Admin/schoolYearTerm_manage_delete.php")==FALSE) {
	...
	if ($gibbonSchoolYearTermID=="") {
		...
		if ($result->rowCount()!=1) {
			print "<div class='error'>" ;
				print "The specified term cannot be found." ;
			print "</div>" ;
		}
		else {
			//Let's go!
			$row=$result->fetch() ;
			?>
			<form method="post" action="<? print $_SESSION[$guid]["absoluteURL"] . "/modules/" . $_SESSION[$guid]["module"] . "/schoolYearTerm_manage_deleteProcess.php?gibbonSchoolYearTermID=$gibbonSchoolYearTermID" ?>">
				<table class='smallIntBorder' cellspacing='0' style="width: 100%">	
					<tr>
						<td> 
							<b>Are you sure you want to delete term "<? print $row["name"] ?>"?</b><br/>
