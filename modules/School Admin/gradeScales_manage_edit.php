...
if (isActionAccessible($guid, $connection2, "/modules/School Admin/gradeScales_manage_edit.php")==FALSE) {
	...
	if ($gibbonScaleID=="") {
		...
		if ($result->rowCount()!=1) {
				...
				<tr>
					...
					<td class="right">
						<select name="numeric" id="numeric" style="width: 302px">
							<option <? if ($row["numeric"]=="Y") { print "selected" ; } ?> value="Y"><? print _('Yes') ?></option>
							<option <? if ($row["numeric"]=="N") { print "selected" ; } ?> value="N"><? print _('No') ?></option>
						</select>
					</td>
				</tr>
				<tr>
			...
			print "<a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . $_SESSION[$guid]["module"] . "/gradeScales_manage_edit_grade_add.php&gibbonScaleID=$gibbonScaleID'><img title='" . _('Add New Record') . "' src='./themes/" . $_SESSION[$guid]["gibbonThemeName"] . "/img/page_new.gif'/></a>" ;
			print "</div>" ;
