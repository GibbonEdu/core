...
if (isActionAccessible($guid, $connection2, '/modules/School Admin/externalAssessments_manage_delete.php') == false) {
    ...
    if ($gibbonExternalAssessmentID == '') {
        ...
        if ($result->rowCount() != 1) {
            ...
            $row = $result->fetch();
            ?>
			<form method="post" action="<?php echo $_SESSION[$guid]['absoluteURL'].'/modules/'.$_SESSION[$guid]['module']."/externalAssessments_manage_deleteProcess.php?gibbonExternalAssessmentID=$gibbonExternalAssessmentID" ?>">
