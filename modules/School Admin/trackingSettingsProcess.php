...
$URL = $session->get('absoluteURL').'/index.php?q=/modules/'.getModuleName($_POST['address']).'/trackingSettings.php';

if (isActionAccessible($guid, $connection2, Resource::fromRoute('School Admin', 'trackingSettings')) == false) {
    $URL .= '&return=error0';
    header("Location: {$URL}");
} else {
    $fail = false;
