...
if (isActionAccessible($guid, $connection2, '/modules/School Admin/inSettings.php') == false) {
    ...
    $settingByScope = getSettingByScope($connection2, 'Individual Needs', 'targetsTemplate', true);
    $row = $form->addRow();
    ...
    $settingByScope = getSettingByScope($connection2, 'Individual Needs', 'teachingStrategiesTemplate', true);
    $row = $form->addRow();
        $row->addLabel($settingByScope['name'], $settingByScope['nameDisplay'])->description($settingByScope['description']);
        $row->addTextArea($settingByScope['name'])->setValue($settingByScope['value']);

    $settingByScope = getSettingByScope($connection2, 'Individual Needs', 'notesReviewTemplate', true);
    $row = $form->addRow();
        $row->addLabel($settingByScope['name'], $settingByScope['nameDisplay'])->description($settingByScope['description']);
        $row->addTextArea($settingByScope['name'])->setValue($settingByScope['value']);
