...
if (isActionAccessible($guid, $connection2, '/modules/School Admin/gradeScales_manage_edit_grade_add.php') == false) {
    ...
    if ($gibbonScaleID == '') {
        ...
        if ($result->rowCount() != 1) {
            ...
            $form = Form::create('gradeScaleGradeAdd', $session->get('absoluteURL').'/modules/'.$session->get('module').'/gradeScales_manage_edit_grade_addMultiProcess.php');
            ...
            $row = $form->addRow();
                $row->addLabel('valueMin', __('Min Value'))->description(__('Must be unique for this grade scale. Duplicate values will be discarded.'));
                $row->addTextField('valueMin')->required()->maxLength(5);

            $row = $form->addRow();
                $row->addLabel('valueMax', __('Max Value'))->description(__('Must be unique for this grade scale. Duplicate values will be discarded.'));
                $row->addTextField('valueMax')->required()->maxLength(5);

            $row = $form->addRow();
                $row->addLabel('descriptorPrefix', __('Descriptor Prefix'))->description(__('Leaving it blank will use the numeric value as the description.'));
                $row->addTextField('descriptorPrefix')->maxLength(45);
