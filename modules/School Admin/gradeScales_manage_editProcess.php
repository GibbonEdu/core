			...
			if ($name=="" OR $nameShort=="" OR $usage=="" OR $active=="" OR $numeric=="") {
				...
				if ($result->rowCount()>0) {
					...
					try {
						...
						$sql="UPDATE gibbonScale SET name=:name, nameShort=:nameShort, `usage`=:usage, lowestAcceptable=NULL, active=:active, `numeric`=:numeric, lowestAcceptable=:lowestAcceptable WHERE gibbonScaleID=:gibbonScaleID" ;
						$result=$connection2->prepare($sql);
						$result->execute($data);
					}
					catch(PDOException $e) { 
