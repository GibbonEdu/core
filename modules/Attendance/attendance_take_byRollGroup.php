...
if (isActionAccessible($guid, $connection2, '/modules/Attendance/attendance_take_byRollGroup.php') == false) {
    ...
    if ($highestAction == false) {
        ...
        //GS//die(var_dump($myRole)); //GS//

        $row = $form->addRow();
            $row->addLabel('gibbonRollGroupID', __('Roll Group'));
            $row->addSelectRollGroup('gibbonRollGroupID', $_SESSION[$guid]['gibbonSchoolYearID'], false, ($myRole == "Teacher" ? $_SESSION[$guid]['gibbonPersonID'] : NULL))->required()->selected($gibbonRollGroupID)->placeholder(); //GS//
                ...
                if (isSchoolOpen($guid, $currentDate, $connection2) == false) {
                    ...
                    if ($rollGroup['attendance'] == 'N') {
                        ...
                        if ($resultRollGroup->rowCount() < 1) {
                            ...
                            //GS//    $col->addSelect('set-all-reason')->fromArray($attendance->getAttendanceReasons())->addClass('m-px');
                            //GS//    $col->addTextField('set-all-comment')->maxLength(255)->addClass('m-px');
