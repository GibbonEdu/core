    ...
    if ($dateStart != '') {
        ...
        try {
            ...
            if ($mode == 'endofday') {
                ...
                $sql = "SELECT c.name, l.reason, count(DISTINCT l.gibbonPersonID) as count, l.date FROM gibbonAttendanceLogPerson l JOIN gibbonAttendanceCode c ON (l.gibbonAttendanceCodeID=c.gibbonAttendanceCodeID) INNER JOIN (SELECT gibbonPersonID, date, MAX(timestampTaken) as maxTimestamp FROM gibbonAttendanceLogPerson WHERE date>=:dateStart AND date<=:dateEnd GROUP BY gibbonPersonID, date) log ON (l.gibbonPersonID=log.gibbonPersonID) ".$rollGroupJoin." WHERE l.date=log.date AND l.timestampTaken=log.maxTimestamp AND l.date>=:dateStart AND l.date<=:dateEnd GROUP BY l.date, c.name, l.reason ORDER BY l.date, c.direction DESC, c.name";
            } else {
                // Count all records
