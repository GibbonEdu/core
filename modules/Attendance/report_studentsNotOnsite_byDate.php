...
if (isActionAccessible($guid, $connection2, "/modules/Attendance/report_studentsNotOnsite_byDate.php")==FALSE) {
	...
	print "<div class='trailHead'><a href='" . $_SESSION[$guid]["absoluteURL"] . "'>" . __($guid, "Home") . "</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . getModuleName($_GET["q"]) . "/" . getModuleEntry($_GET["q"], $connection2, $guid) . "'>" . __($guid, getModuleName($_GET["q"])) . "</a> > </div><div class='trailEnd'>" . __($guid, 'Students Not Onsite') . "</div>" ;
	print "</div>" ;
	...
	if ($currentDate!="") {
		...
		if ($result->rowCount()<1) {
			...
			if ($result->rowCount()<1) {
				...
				print "<a target='_blank' href='" . $_SESSION[$guid]["absoluteURL"] . "/report.php?q=/modules/" . $_SESSION[$guid]["module"] . "/report_studentsNotOnsite_byDate_print.php&currentDate=" . dateConvertBack($guid, $currentDate) . "'>" .  __($guid, 'Print') . "<img style='margin-left: 5px' title='" . __($guid, 'Print') . "' src='./themes/" . $_SESSION[$guid]["gibbonThemeName"] . "/img/print.png'/></a>" ;
				print "</div>" ;
