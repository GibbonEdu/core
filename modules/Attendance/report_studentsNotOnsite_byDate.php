...
if (isActionAccessible($guid, $connection2, "/modules/Attendance/report_studentsNotOnsite_byDate.php")==FALSE) {
	...
	print "<div class='trailHead'><a href='" . $_SESSION[$guid]["absoluteURL"] . "'>" . _("Home") . "</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . _(getModuleName($_GET["q"])) . "/" . getModuleEntry($_GET["q"], $connection2, $guid) . "'>" . getModuleName($_GET["q"]) . "</a> > </div><div class='trailEnd'>" . _('Students Not Onsite') . "</div>" ;
	print "</div>" ;
	...
	if ($currentDate!="") {
		...
		if ($result->rowCount()<1) {
			...
			if ($result->rowCount()<1) {
				...
				print "<a target='_blank' href='" . $_SESSION[$guid]["absoluteURL"] . "/report.php?q=/modules/" . $_SESSION[$guid]["module"] . "/report_studentsNotOnsite_byDate_print.php&currentDate=" . dateConvertBack($guid, $currentDate) . "'><img title='" . _('Print') . "' src='./themes/" . $_SESSION[$guid]["gibbonThemeName"] . "/img/print.png'/></a>" ;
				print "</div>" ;
