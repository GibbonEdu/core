            ...
            foreach ($report as $reportEntry) {
                if ($reportEntry['contactType'] == 'Email') {
                    ...
                    if ($reportEntry['targetType'] == 'Mailing List') {
                        $bodyOut .= '<br/><hr/><br/><i>'.__('{linkOpen}Click here to manage your subscriptions or unsubscribe{linkClose}', ['linkOpen' => '<a href="'.$session->get('absoluteURL').'/index.php?q=/modules/Messenger/mailingListRecipients_manage_subscribe.php&mode=unsubscribe&email='.$reportEntry['contactDetail'].'&key='.$reportEntry['unsubscribeKey'].'">', 'linkClose' => '</a>'])."</i>";
                    }
