...
if (isActionAccessible($guid, $connection2, "/modules/Messenger/cannedResponse_manage_delete.php")==FALSE) {
	...
	if ($gibbonMessengerCannedResponseID=="") {
		...
		if ($result->rowCount()!=1) {
			...
			$row=$result->fetch() ;
			?>
			<form method="post" action="<?php print $_SESSION[$guid]["absoluteURL"] . "/modules/" . $_SESSION[$guid]["module"] . "/cannedResponse_manage_deleteProcess.php?gibbonMessengerCannedResponseID=$gibbonMessengerCannedResponseID" ?>">
