...
if (isActionAccessible($guid, $connection2, '/modules/Activities/report_notEnrolled.php') == false) {
    // Access denied
    $page->addError(__('You do not have access to this action.'));
} else {
    // Proceed!
    $page->breadcrumbs->add(__('Students Not Enrolled'));
    ...
    $unenrolled = $enrolmentGateway->queryUnenrolledStudentsByCategory($criteria, $params['gibbonActivityCategoryID']);
    
    // DATA TABLE
    $table = ReportTable::createPaginated('report_notEnrolled', $criteria)->setViewMode($viewMode, $session);

    $table->setTitle(__('Students Not Enrolled'));
