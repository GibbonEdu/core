...
if (isActionAccessible($guid, $connection2, "/modules/Activities/activities_view.php")==FALSE) {
	...
	if ($highestAction==FALSE) {
		...
		print "<div class='trailHead'><a href='" . $_SESSION[$guid]["absoluteURL"] . "'>" . _("Home") . "</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . _(getModuleName($_GET["q"])) . "/" . getModuleEntry($_GET["q"], $connection2, $guid) . "'>" . getModuleName($_GET["q"]) . "</a> > </div><div class='trailEnd'>" . _('View Activities') . "</div>" ;
		print "</div>" ;
